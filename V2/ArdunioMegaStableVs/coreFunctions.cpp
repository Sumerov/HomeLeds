...
void introSetup() {
  ...
  fadeToogleToColor(245, 245, 245);
  delay(1000);
  fadeToogleToColor(245, 0, 0);
  delay(500);
  fadeToogleToColor(0, 245, 0);
  delay(500);
  fadeToogleToColor(0, 0, 245);
  delay(500);
      ...
      if(controlBools[i]) {
        ...
        switch(i) {
          case 0: {  // TURNOFF
            ...
            AnToPWMOn[4] = false;
            fadeToogleToColor(newRGB[0], newRGB[1], newRGB[2]);
            break;
          }
          case 1: { // rgbChanged
            for(uint8_t i=0; i<4; i++) { AnToLedsOn[i] = true; }
            fadeToogleToColor(newRGB[0], newRGB[1], newRGB[2]);
            break;
          }
